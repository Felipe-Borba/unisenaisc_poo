...
public class InputFieldsFrame extends JFrame {
	...
	private static final int NUM_CHKBOX = 5;
	private static final int NUM_RADBTN = 5;
	...
	private void initialize() {
		...
		addComponent(toggleButton, 8, 1, 1, 1);

		label = new JLabel("CheckBox:");
		addComponent(label, 9, 0, 1, 1);
		panel = new JPanel();
		for (int i = 1; i <= NUM_CHKBOX; i++) {
		...
		JList<String> list = new JList<>(LIST_ITEMS);
		...
		String[] columnNames = { "Nome", "Sobrenome", "E-mail" };
		String[][] data = { 
		...
		table.setPreferredScrollableViewportSize(
				new Dimension(table.getPreferredSize().width, table.getRowHeight() * 3));
	...
	private void addComponent(JComponent c, int row, int col, int w, int h) {
...
} // fim da classe InputFieldsFrame
